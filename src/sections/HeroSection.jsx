...
export default function HeroSection({ onMenuClick, onNavigate }) {
  return (
    <section id="hero" tabIndex="-1" className="portfolio-section relative h-[1080px] overflow-hidden bg-surface-warm-beige outline-none" aria-labelledby="hero-title">
      ...
      <img className="absolute left-[149px] top-[12px] h-[1121px] w-[747px] object-cover" src="/assets/images/hero-portrait.png" alt="송원일 프로필 사진" fetchPriority="high" />
      ...
      <h1 id="hero-title" className="absolute left-[945px] top-[144px] h-[490px] w-[810px] text-center text-hero font-semibold">
        <span className="absolute left-0 top-0 w-full whitespace-nowrap text-text-primary">ONE SHOT에서</span>
        <span className="absolute left-0 top-[137px] w-full whitespace-nowrap text-surface-warm-beige [text-shadow:-1px_-1px_0_#121212,1px_-1px_0_#121212,-1px_1px_0_#121212,1px_1px_0_#121212]">ONE FRAME을,</span>
        <span className="absolute left-0 top-[283px] w-full whitespace-nowrap text-text-primary">섬세하게 다듬다.</span>
      </h1>
      <p className="absolute left-[960px] top-[646px] m-0 whitespace-nowrap text-[36px] font-medium leading-[47px] text-text-primary">흐림 속에서, 선명한 장면을 찾습니다.</p>
      <div className="absolute left-[1154px] top-[773px] flex gap-space-20">
        <CtaButton>이력서</CtaButton>
        <CtaButton variant="outlined">깃허브</CtaButton>
      </div>
    </section>
  );
}
